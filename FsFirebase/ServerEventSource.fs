...
type EventSourceProcessor(?initReconnectionTime, ?initLastId) =
    ...
    member x.Start uri uriModifier =
        let networkSink = _createNetworkStream (stream, (fun id -> lastId <- id), (fun time -> reconnectionTime <- time))
        let message = _createEventSourceMessage uri lastId
        async {
            let! response = _connectEventSourceServer message
            match response with
            | OK networkStream ->
                Observable.observeStream (Async.RunSynchronously) networkSink networkStream
